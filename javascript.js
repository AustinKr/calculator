...
function removeDigitFromEntry(entry, digitCount = 1, defaulToNull = true) {
  ...
  if (newCount <= 0) return defaulToNull ? null : "0";
...
function getCurrentEntryID() {
  for (let i = 0; i < entries.length; i++) {
    if (isEntryEmpty(entries[i])) return i;
  }
...
function enterDigit(digit) {
  ...
  if (currentID <= 1) setEntry(0, digit);
  else if (currentID > 1) setEntry(2, digit);
...
function main() {
  document.querySelector("body").addEventListener("keyup", (event) => {
    switch (event.key) {
      ...
      case "=":
        ...
        break;
      case "Backspace":
        if (event.shiftKey) enterClear();
        else enterDeleteDigit();
        break;
  ...
  document
    ...
    .addEventListener("click", () => enterDeleteDigit());
  ...
  document
    ...
    .addEventListener("click", () => enterClear());
  document
    .getElementById("evaluate-button")
    .addEventListener("click", () => enterEvaluate());
  document
    .getElementById("decimal-button")
    .addEventListener("click", () => (shouldAddDecimalPoint = true));
